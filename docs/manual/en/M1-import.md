...
- **"The unit is not part of ΟΚΥπΥ."** The row belongs to a body outside the organisation — the Ambulance Service, for instance, which left ΟΚΥπΥ on 19/09/2026. The project does not go into the register, and do not add the spelling as an alias: the unit must not exist here. Take the row out of the file, or send it to the body that now runs the service, and run the import again.
- **"The run stayed a dry run."** One of the blocking rules (V04, V06, V07, V10, V11, V12, V13, V15) did not pass. The report says which, and on which row. Fix the file and run again.
