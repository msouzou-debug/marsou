...
The sign-in page identifies you through your ΟΚΥπΥ account and shows you only the units you belong to. In development, where there is no Entra ID tenant, you sign in as one of five sample users instead.
...
The five accounts: `admin@ecapital.test` (administrator, all units), `estates.nicosia@ecapital.test` (head of estates, Nicosia), `engineer.larnaca@ecapital.test` (project engineer, Larnaca), `clinical.nicosia@ecapital.test` (clinical approver, Nicosia), `auditor@ecapital.test` (auditor, all units, read-only).
...
- **An entry is refused because the account only reads.** The auditor account cannot change anything. Use an account with write access.
