...
- **Three glossary terms, updated by the owner (19/09/2026).** CAPEX-02 §7 fixes «Πιστοποίηση πληρωμής» for a payment certificate and «Κρατήσεις» for retention. The API's error and warning strings use **«Πιστοποιητικό πληρωμής»** and **«Παρακράτηση»**, and **«Δεδουλευμένα»** for the year-end accruals, which §7 has no entry for. «Δαπάνες» for actuals, «Δεσμεύσεις» for commitments and «Πρόβλεψη τελικού κόστους» for the forecast are unchanged. The screens should follow the same three words so the interface and the error bodies do not disagree.
