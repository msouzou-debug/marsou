...
/**
 * Πάγια — the asset register the M4 seed writes (R26–R30, R45, CAPEX-01 §15).
 *
 * Twenty-five assets across Nicosia General and Larnaca General: the
 * air-handling units with their fans under them, the chillers, the
 * medical-gas manifold that feeds the ICU and the theatres, the LV main board
 * that feeds the building, a lift, a fire panel, two biomedical items (R45)
 * and one chiller that has been disposed of and stays in the register because
 * its history is the argument for the one that replaced it.
 *
 * Two things in here are deliberate and are worth reading twice.
 *
 *  - **`system` and `servesAreaIds` are set on the medical gas, the
 *    electrical, the fire and the water assets, and not on the HVAC ones.**
 *    That is not an oversight: `GET /areas/impact` asks the register first
 *    and falls back to `system_feed` **per system** (ADR-0028), and a seeded
 *    register that exercised only one of the two paths would leave the other
 *    untested. It is also what a half-surveyed hospital actually looks like —
 *    the gas and the boards have been recorded, the HVAC risers have not.
 *  - **The replacement years are spread over 2027–2035** so the forecast has
 *    a shape rather than a spike, and the criticality and condition are
 *    spread so S21's priority rank has something to sort.
 *
 * NO PATIENT DATA. A ventilator and an anaesthesia machine are in here as
 * equipment: a make, a model, a serial number, a room and a date. Nothing
 * about anybody who was ever connected to one.
 */
export interface SeedAssetReading {
  readingType: string;
  value: number;
  unit: string | null;
  /** Relative to the moment the seed runs — a reading is a fact about a day. */
  daysAgo: number;
}

export interface SeedAssetDocument {
  kind: "OM_MANUAL" | "CERT" | "COMMISSIONING" | "WARRANTY" | "DRAWING" | "PHOTO";
  titleEl: string;
  /** eArchive's own number. Seeded as already filed. */
  protocolNumber: string;
  filename: string;
  mime: string;
  /** The operational copy's stand-in; the seed writes no bytes to disk. */
  body: string;
}

export interface SeedAsset {
  /** Unique within the seed; also how a child names its parent. */
  key: string;
  orgUnitId: string;
  /** Null = the whole building or external plant, e.g. a chiller on the roof. */
  ...
  /** The seeded project of the same unit this came out of, by its Greek title. */
  projectTitleEl: string | null;
  /** True where that project has a contract and the asset was delivered under it. */
  ...
  criticality: number;
  condition: "A" | "B" | "C" | "D" | "E" | null;
  parentKey: string | null;
  servesAreaCodes: string[];
  system: PermitSystem | null;
  costCentre: string | null;
  sapAssetNo: string | null;
  status: "IN_SERVICE" | "OUT_OF_SERVICE" | "DISPOSED" | "PLANNED";
  readings: SeedAssetReading[];
  documents: SeedAssetDocument[];
}
...
export const seedAssets: SeedAsset[] = [
  // ------------------------------------------------ Λευκωσία, κλιματισμός --
  {
    key: "ngh-ahu-1",
    orgUnitId: "nicosia-general",
    areaCode: "PLT-01",
    nameEl: "Κλιματιστική μονάδα ΚΚΜ-1",
    assetClass: "HVAC",
    manufacturer: "Systemair",
    model: "DV-100",
    serialNo: "SA-2018-11421",
    installedDate: "2018-03-12",
    commissionedDate: "2018-05-02",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 96000,
    warrantyEnd: "2021-05-02",
    expectedLifeYears: 20,
    replacementYear: 2033,
    replacementCostEst: 145000,
    criticality: 2,
    condition: "C",
    parentKey: null,
    servesAreaCodes: [],
    system: "HVAC",
    costCentre: "CC-NGH-TEC",
    sapAssetNo: "ANG-100001",
    status: "IN_SERVICE",
    readings: [
      { readingType: "RUN_HOURS", value: 61200, unit: "h", daysAgo: 120 },
      { readingType: "RUN_HOURS", value: 63840, unit: "h", daysAgo: 10 },
    ],
    documents: [],
  },
  {
    ...
    orgUnitId: "nicosia-general",
    areaCode: "PLT-01",
    nameEl: "Ανεμιστήρας προσαγωγής ΚΚΜ-1",
    assetClass: "HVAC",
    manufacturer: "Ziehl-Abegg",
    model: "RH50V",
    serialNo: "ZA-2018-0087",
    installedDate: "2018-03-12",
    commissionedDate: "2018-05-02",
    projectTitleEl: null,
    withContract: false,
  ...
  },
  {
    ...
    orgUnitId: "nicosia-general",
    areaCode: "PLT-01",
    nameEl: "Κλιματιστική μονάδα ΚΚΜ-2",
    assetClass: "HVAC",
    manufacturer: "Systemair",
    model: "DV-80",
    serialNo: "SA-2019-13007",
    installedDate: "2019-06-04",
    commissionedDate: "2019-07-18",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 74000,
    warrantyEnd: "2022-07-18",
    expectedLifeYears: 20,
    replacementYear: 2034,
    replacementCostEst: 118000,
    criticality: 2,
    condition: "B",
    parentKey: null,
    servesAreaCodes: [],
  ...
  },
  {
    ...
    orgUnitId: "nicosia-general",
    areaCode: "PLT-01",
    nameEl: "Ανεμιστήρας απαγωγής ΚΚΜ-2",
    assetClass: "HVAC",
    manufacturer: "Ziehl-Abegg",
    model: "RH45V",
    serialNo: "ZA-2019-0142",
    installedDate: "2019-06-04",
    commissionedDate: "2019-07-18",
    projectTitleEl: null,
    withContract: false,
  ...
  },
  {
    ...
    orgUnitId: "nicosia-general",
    areaCode: null,
    nameEl: "Ψύκτης Ψ-1, δώμα κτιρίου Α",
    assetClass: "HVAC",
    manufacturer: "Carrier",
    model: "30XA-802",
    serialNo: "CR-2017-4410",
    installedDate: "2017-04-20",
    commissionedDate: "2017-06-01",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 218000,
    warrantyEnd: "2020-06-01",
    expectedLifeYears: 18,
    replacementYear: 2031,
    replacementCostEst: 305000,
    criticality: 1,
    condition: "D",
    parentKey: null,
    servesAreaCodes: [],
    system: "HVAC",
    costCentre: "CC-NGH-TEC",
    sapAssetNo: "ANG-100005",
    status: "IN_SERVICE",
    readings: [
      { readingType: "RUN_HOURS", value: 48100, unit: "h", daysAgo: 180 },
      { readingType: "CONDENSER_APPROACH", value: 3.4, unit: "K", daysAgo: 14 },
    ],
    documents: [],
  },
  {
    ...
    orgUnitId: "nicosia-general",
    areaCode: null,
    nameEl: "Ψύκτης Ψ-2, δώμα κτιρίου Α (αποσυρμένος)",
    assetClass: "HVAC",
    manufacturer: "Trane",
    model: "RTAC-300",
    serialNo: "TR-2004-0912",
    installedDate: "2004-09-01",
    commissionedDate: "2004-11-15",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 132000,
    warrantyEnd: "2007-11-15",
    expectedLifeYears: 18,
    ...
    system: "HVAC",
    costCentre: "CC-NGH-TEC",
    sapAssetNo: "ANG-100006",
    // R26: it has left the estate and stays in the register, because its
    // history is the argument for the one that replaced it.
    status: "DISPOSED",
    readings: [],
    documents: [],
  },
  // ------------------------------------- Λευκωσία, αέρια, ρεύμα, νερό, φωτιά --
  {
    key: "ngh-mgas-manifold",
    orgUnitId: "nicosia-general",
    areaCode: "PLT-01",
    nameEl: "Συστοιχία ιατρικών αερίων Α",
    assetClass: "MEDICAL_GAS",
    manufacturer: "BeaconMedaes",
    model: "MM-2x10",
    serialNo: "BM-2020-0331",
    installedDate: "2020-02-10",
    commissionedDate: "2020-04-06",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 64000,
    ...
    replacementCostEst: 92000,
    criticality: 1,
    condition: "B",
    parentKey: null,
    // §6.1: what a shutdown of it interrupts. This is the row that takes
    // over from `system_feed` for medical gas at Nicosia (ADR-0028).
    servesAreaCodes: ["ICU-01", "THE-01"],
    system: "MEDICAL_GAS",
    costCentre: "CC-NGH-TEC",
    sapAssetNo: "ANG-100007",
    status: "IN_SERVICE",
    readings: [
      { readingType: "LINE_PRESSURE", value: 4.2, unit: "bar", daysAgo: 60 },
      { readingType: "LINE_PRESSURE", value: 4.1, unit: "bar", daysAgo: 7 },
    ],
    documents: [
      {
        kind: "COMMISSIONING",
        titleEl: "Φάκελος παραλαβής συστοιχίας ιατρικών αερίων Α",
        protocolNumber: "ΤΥ/2020/00418",
        filename: "commissioning-medical-gas-a.pdf",
        mime: "application/pdf",
        body: "eCapital seed — commissioning pack for the Nicosia medical gas manifold A.",
      },
      {
        kind: "CERT",
        titleEl: "Πιστοποιητικό καθαρότητας ιατρικού αέρα",
        protocolNumber: "ΤΥ/2020/00419",
        filename: "cert-medical-air-purity.pdf",
        mime: "application/pdf",
        body: "eCapital seed — medical air purity certificate for manifold A.",
      },
    ],
  },
  {
    ...
    orgUnitId: "nicosia-general",
    areaCode: "PLT-01",
    nameEl: "Κεντρικός πίνακας χαμηλής τάσης κτιρίου Α",
    assetClass: "ELECTRICAL",
    manufacturer: "Schneider Electric",
    model: "Prisma P",
    serialNo: "SE-2006-0021",
    installedDate: "2006-01-15",
    commissionedDate: "2006-05-30",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 188000,
    warrantyEnd: null,
  ...
  },
  {
    ...
    orgUnitId: "nicosia-general",
    areaCode: null,
    nameEl: "Ηλεκτροπαραγωγό ζεύγος Η/Ζ-1",
    assetClass: "ELECTRICAL",
    manufacturer: "Caterpillar",
    model: "C32",
    serialNo: "CAT-2012-7781",
    installedDate: "2012-07-02",
    commissionedDate: "2012-09-14",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 265000,
    warrantyEnd: "2015-09-14",
    expectedLifeYears: 25,
    replacementYear: 2035,
    replacementCostEst: 410000,
    criticality: 1,
    condition: "B",
    parentKey: null,
    servesAreaCodes: [],
    system: "ELECTRICAL",
    costCentre: "CC-NGH-TEC",
    sapAssetNo: "ANG-100009",
    status: "IN_SERVICE",
    readings: [
      { readingType: "RUN_HOURS", value: 1840, unit: "h", daysAgo: 90 },
      { readingType: "RUN_HOURS", value: 1902, unit: "h", daysAgo: 3 },
    ],
    documents: [],
  },
  {
    ...
    orgUnitId: "nicosia-general",
    areaCode: "PLT-01",
    nameEl: "Σύστημα αδιάλειπτης παροχής UPS-1",
    assetClass: "ELECTRICAL",
    manufacturer: "Riello",
    model: "MPW 120",
    serialNo: "RI-2019-0455",
    installedDate: "2019-11-05",
    commissionedDate: "2019-12-20",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 48000,
    warrantyEnd: "2022-12-20",
    expectedLifeYears: 12,
    replacementYear: 2027,
    replacementCostEst: 62000,
    criticality: 2,
    condition: "C",
    parentKey: null,
    servesAreaCodes: [],
  ...
  },
  {
    ...
    orgUnitId: "nicosia-general",
    areaCode: "OFF-01",
    nameEl: "Κεντρικός πίνακας πυρανίχνευσης",
    assetClass: "FIRE",
    manufacturer: "Siemens",
    model: "FC722",
    serialNo: "SI-2021-2210",
    installedDate: "2021-05-11",
    commissionedDate: "2021-07-01",
    // R27: it came out of the fire-detection upgrade that is in the register.
    projectTitleEl: "Αναβάθμιση συστήματος πυρανίχνευσης",
    withContract: true,
    capitalCost: 58000,
    warrantyEnd: "2026-07-01",
    expectedLifeYears: 15,
    replacementYear: 2033,
    replacementCostEst: 78000,
    criticality: 1,
    condition: "A",
    parentKey: null,
    servesAreaCodes: ["OPD-01", "PLT-01", "OFF-01", "THE-01", "ICU-01", "WRD-01"],
    system: "FIRE",
    costCentre: "CC-NGH-TEC",
    sapAssetNo: "ANG-100011",
    status: "IN_SERVICE",
    readings: [],
    documents: [],
  },
  {
    key: "ngh-sprinkler-pump",
    orgUnitId: "nicosia-general",
    areaCode: "PLT-01",
    nameEl: "Αντλητικό συγκρότημα πυρόσβεσης",
    assetClass: "FIRE",
    manufacturer: "Grundfos",
    model: "NKF 80-250",
    serialNo: "GR-2015-3390",
    installedDate: "2015-08-19",
    commissionedDate: "2015-10-05",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 41000,
    warrantyEnd: "2018-10-05",
    expectedLifeYears: 20,
    replacementYear: 2032,
    replacementCostEst: 58000,
    criticality: 2,
    condition: "C",
    parentKey: null,
    servesAreaCodes: [],
  ...
  },
  {
    ...
    orgUnitId: "nicosia-general",
    areaCode: "PLT-01",
    nameEl: "Πιεστικό συγκρότημα ύδρευσης",
    assetClass: "WATER",
    manufacturer: "Wilo",
    model: "COR-3 Helix",
    serialNo: "WI-2016-1102",
    installedDate: "2016-02-22",
    commissionedDate: "2016-04-11",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 27000,
    warrantyEnd: "2019-04-11",
    expectedLifeYears: 15,
    replacementYear: 2028,
    replacementCostEst: 39000,
    criticality: 2,
    condition: "C",
    parentKey: null,
    // Deliberately blank: nobody has surveyed what the booster set feeds, and
    // a register in that state is what `GET /areas/impact` has to cope with.
    servesAreaCodes: [],
    system: "WATER",
    costCentre: "CC-NGH-TEC",
    sapAssetNo: "ANG-100013",
    status: "IN_SERVICE",
    readings: [],
    documents: [],
  },
  {
    ...
    orgUnitId: "nicosia-general",
    areaCode: null,
    nameEl: "Ανελκυστήρας Α1, κτίριο Α",
    assetClass: "LIFT",
    manufacturer: "Kleemann",
    model: "Atlas MRL",
    serialNo: "KL-2013-0774",
    installedDate: "2013-03-08",
    commissionedDate: "2013-06-20",
    projectTitleEl: "Αντικατάσταση ανελκυστήρων",
    withContract: true,
    capitalCost: 92000,
    warrantyEnd: "2016-06-20",
    expectedLifeYears: 25,
    replacementYear: 2034,
    replacementCostEst: 128000,
    criticality: 2,
    condition: "B",
    parentKey: null,
    servesAreaCodes: [],
  ...
  },
  {
    ...
    orgUnitId: "nicosia-general",
    areaCode: "ICU-01",
    nameEl: "Αναπνευστήρας ΜΕΘ κλίνης 3",
    // R45: biomedical equipment is in the same register as the building assets.
    assetClass: "BIOMEDICAL",
    manufacturer: "Dräger",
    model: "Evita V600",
    serialNo: "DR-2022-5514",
    installedDate: "2022-01-24",
    commissionedDate: "2022-02-14",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 34000,
  ...
  },
  {
    ...
    orgUnitId: "nicosia-general",
    areaCode: "THE-01",
    nameEl: "Μηχάνημα αναισθησίας χειρουργείου 1",
    assetClass: "BIOMEDICAL",
    manufacturer: "GE Healthcare",
    model: "Carestation 750",
    serialNo: "GE-2021-8830",
    installedDate: "2021-09-30",
    commissionedDate: "2021-11-08",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 46000,
  ...
  },
  {
    ...
    orgUnitId: "nicosia-general",
    areaCode: "OFF-01",
    nameEl: "Ικρίωμα δικτύου κτιρίου Α",
    assetClass: "IT",
    manufacturer: "APC",
    model: "NetShelter SX",
    serialNo: "AP-2020-1177",
    installedDate: "2020-10-12",
    commissionedDate: "2020-11-02",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 18000,
    warrantyEnd: "2023-11-02",
    expectedLifeYears: 12,
    replacementYear: 2029,
    replacementCostEst: 24000,
    criticality: 3,
    condition: "B",
    parentKey: null,
    servesAreaCodes: [],
  ...
  },
  {
    ...
    orgUnitId: "nicosia-general",
    areaCode: null,
    nameEl: "Στεγάνωση δώματος κτιρίου Α",
    assetClass: "BUILDING_FABRIC",
    manufacturer: null,
    model: null,
    serialNo: null,
    installedDate: "2006-05-30",
    commissionedDate: "2006-05-30",
    projectTitleEl: "Επισκευή στέγης χειρουργικού τομέα",
    withContract: false,
    capitalCost: null,
    warrantyEnd: null,
    expectedLifeYears: 25,
    replacementYear: 2027,
    replacementCostEst: 260000,
    criticality: 3,
    condition: "D",
    parentKey: null,
    servesAreaCodes: [],
  ...
  // ---------------------------------------------------------- Λάρνακα --
  {
    ...
    orgUnitId: "larnaca-general",
    areaCode: "PLT-01",
    nameEl: "Κλιματιστική μονάδα ΚΚΜ-Λ1",
    assetClass: "HVAC",
    manufacturer: "Systemair",
    model: "DV-60",
    serialNo: "SA-2012-09876",
    installedDate: "2012-04-02",
    commissionedDate: "2012-06-15",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 61000,
    warrantyEnd: "2015-06-15",
    expectedLifeYears: 20,
    replacementYear: 2029,
    replacementCostEst: 96000,
    criticality: 2,
    condition: "D",
    parentKey: null,
    servesAreaCodes: [],
    system: "HVAC",
    costCentre: "CC-LAR-TEC",
    sapAssetNo: "ALA-200001",
    status: "IN_SERVICE",
    readings: [
      { readingType: "RUN_HOURS", value: 92400, unit: "h", daysAgo: 150 },
      { readingType: "RUN_HOURS", value: 95760, unit: "h", daysAgo: 12 },
    ],
    documents: [],
  },
  {
    ...
    orgUnitId: "larnaca-general",
    areaCode: "PLT-01",
    nameEl: "Ανεμιστήρας προσαγωγής ΚΚΜ-Λ1",
    assetClass: "HVAC",
    manufacturer: "Ziehl-Abegg",
    model: "RH40V",
    serialNo: "ZA-2012-0031",
    installedDate: "2012-04-02",
    commissionedDate: "2012-06-15",
    projectTitleEl: null,
    withContract: false,
  ...
  },
  {
    ...
    orgUnitId: "larnaca-general",
    areaCode: null,
    nameEl: "Ψύκτης Ψ-Λ1, δώμα νέας πτέρυγας",
    assetClass: "HVAC",
    manufacturer: "Daikin",
    model: "EWAD-TZ",
    serialNo: "DA-2015-6621",
    installedDate: "2015-05-18",
    commissionedDate: "2015-07-30",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 174000,
    warrantyEnd: "2018-07-30",
    expectedLifeYears: 18,
    replacementYear: 2030,
    replacementCostEst: 248000,
    criticality: 1,
    condition: "C",
    parentKey: null,
    servesAreaCodes: [],
  ...
  },
  {
    ...
    orgUnitId: "larnaca-general",
    areaCode: "PLT-01",
    nameEl: "Κεντρικός πίνακας χαμηλής τάσης νέας πτέρυγας",
    assetClass: "ELECTRICAL",
    manufacturer: "ABB",
    model: "MNS",
    serialNo: "AB-2011-0044",
    installedDate: "2011-02-01",
    commissionedDate: "2011-09-12",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 142000,
    warrantyEnd: null,
  ...
  },
  {
    ...
    orgUnitId: "larnaca-general",
    areaCode: "PLT-01",
    nameEl: "Κεντρικός πίνακας πυρανίχνευσης νέας πτέρυγας",
    assetClass: "FIRE",
    manufacturer: "Honeywell",
    model: "NFS2-3030",
    serialNo: "HW-2017-4402",
    installedDate: "2017-03-15",
    commissionedDate: "2017-05-22",
    projectTitleEl: "Αναβάθμιση δικτύου πυρόσβεσης",
    withContract: true,
    capitalCost: 52000,
    warrantyEnd: "2022-05-22",
    expectedLifeYears: 15,
    replacementYear: 2031,
    replacementCostEst: 71000,
    criticality: 1,
    condition: "B",
    parentKey: null,
    servesAreaCodes: ["PLT-01", "OPD-01", "THE-01", "WRD-01"],
  ...
  },
  {
    ...
    orgUnitId: "larnaca-general",
    areaCode: null,
    nameEl: "Ανελκυστήρας Λ1, νέα πτέρυγα",
    assetClass: "LIFT",
    manufacturer: "Kleemann",
    model: "Atlas Bed",
    serialNo: "KL-2011-0220",
    installedDate: "2011-06-10",
    commissionedDate: "2011-09-12",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 88000,
    warrantyEnd: "2014-09-12",
    expectedLifeYears: 25,
    replacementYear: 2033,
    replacementCostEst: 124000,
    criticality: 2,
    condition: "C",
    parentKey: null,
    servesAreaCodes: [],
  ...
  },
  {
    ...
    orgUnitId: "larnaca-general",
    areaCode: "WRD-01",
    nameEl: "Αναπνευστήρας θαλάμου Β1",
    assetClass: "BIOMEDICAL",
    manufacturer: "Hamilton Medical",
    model: "C6",
    serialNo: "HM-2023-1190",
    installedDate: "2023-04-11",
    commissionedDate: "2023-05-03",
    projectTitleEl: null,
    withContract: false,
    capitalCost: 31000,
    warrantyEnd: "2028-05-03",
    expectedLifeYears: 10,
    replacementYear: 2033,
    replacementCostEst: 38000,
    criticality: 1,
    condition: "A",
    parentKey: null,
    servesAreaCodes: [],
  ...
  },
  {
    ...
    orgUnitId: "larnaca-general",
    areaCode: "OPD-01",
    nameEl: "Ακτινολογικό συγκρότημα εξωτερικών ιατρείων",
    assetClass: "BIOMEDICAL",
    manufacturer: "Siemens Healthineers",
    model: "Ysio Max",
    serialNo: "SH-2026-0012",
    installedDate: null,
    ...
    costCentre: "CC-LAR-OPD",
    sapAssetNo: null,
    // Bought and not yet commissioned: the project is IN_PROGRESS.
    status: "PLANNED",
    readings: [],
    documents: [],
  },
