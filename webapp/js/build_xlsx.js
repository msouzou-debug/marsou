...
function journalLines(section, lookup) {
  /* One provider unit's credit lines: {kostl, aufnr, text, amount}, ordered by
   * cost centre then internal order — the order the service's own journal
   * uses.  Whatever the clinic split does not cover (claims vs SRA, adjustment
  ...
  for (const sh of clinicShares([section])) {
    ...
    const key = [kostl, aufnr, clinicKey(sh.clinic),
                 kostl ? '' : sh.speciality].join('|');
    buckets.set(key, round2((buckets.get(key) || 0) + sh.amount));
    labels.set(key, [text, kostl, aufnr, clinicKey(sh.clinic)]);
  ...
  const keys = [...buckets.keys()].sort((a, b2) => {
    const [, ka, oa, ca] = labels.get(a);
    const [, kb, ob, cb] = labels.get(b2);
    if (!ka !== !kb) return ka ? -1 : 1;      // coded lines first
    return cmp(ka, kb) || cmp(oa, ob) || cmp(ca, cb);
  });
  const out = keys.map((key) => {
    const [text, kostl, aufnr] = labels.get(key);
    return { kostl, aufnr, text, amount: buckets.get(key) };
  });
